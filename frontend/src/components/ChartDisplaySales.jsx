...
const ChartDisplaySales = ({ data, title, futureData = [], colorMap }) => {
    if (!data.length) {
    ...
    const datasets = products.flatMap((product) => {
      ...
      const historicalDataset = {
        label: `${product} (Historical Sales)`,
        data: timestamps.map((timestamp) => {
          ...
          // 將所有 matches 的 salesAmount 加總
          const totalSales = matches.reduce((acc, item) => {
            const val = parseFloat(item.salesAmount) || 0;
            return acc + val;
          }, 0);
      ...
      const predictionDataset = {
            label: `${product} (Prediction Sales)`,
            data: timestamps.map((timestamp) => {
              const match = futureData.find(
                (item) => item.productName === product && item.timestamp === timestamp
              );
              return match ? parseFloat(match.salesAmount) : null;
            }),
            borderColor: productColor,
            borderDash: [5, 5], //虛線模式
            spanGaps: true,
  ...
  export default ChartDisplaySales;
